...
contract MetadataInternal is OwnableInternal {
    ...
    function _setMetadata(uint256 _tokenId, MetadataStorage.Metadata calldata _metadata) internal onlyOwner {
        ...
        metadataStore.metadata[_tokenId].animation_url = _metadata.animation_url;
        // metadata.attributes = _metadata.attributes;

        uint256 attributesLength = _metadata.attributes.length;
        uint256 i = 0;
        while (i < attributesLength) {
            metadataStore.metadata[_tokenId].attributes.push(_metadata.attributes[i]);
            i++;
        }
    }
