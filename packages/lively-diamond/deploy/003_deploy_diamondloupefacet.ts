...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (DiamondLoupeFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("DiamondLoupeFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live && chainId !== "1337") {
    ...
    const artifact = await deployments.getArtifact("DiamondLoupeFacet");
