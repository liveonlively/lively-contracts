...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (CrossmintFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("CrossmintFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live) {
    ...
    console.log("Verifying CrossmintFacet...");
    const artifact = await deployments.getArtifact("CrossmintFacet");
