...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (ERC721AFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("ERC721AFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live && chainId !== "1337") {
    ...
    console.log("Verifying ERC721AFacet...");
    const artifact = await deployments.getArtifact("ERC721AFacet");
