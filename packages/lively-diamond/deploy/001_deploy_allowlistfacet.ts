...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (AllowListFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("AllowListFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live && chainId !== "1337") {
    ...
    console.log("Verifying AllowListFacet...");
    const artifact = await deployments.getArtifact("AllowListFacet");
