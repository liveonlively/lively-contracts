...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (DiamondEtherscanFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("DiamondEtherscanFacet", deployOptions);
    } catch (e) {
  ...
  console.log("DiamondEtherscanFacet: ", {
  ...
  if (network.live) {
    ...
    console.log("Verifying DiamondEtherscanFacet...");
    const artifact = await deployments.getArtifact("DiamondEtherscanFacet");
