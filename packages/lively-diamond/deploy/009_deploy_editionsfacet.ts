...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (EditionsFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("EditionsFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live && chainId !== "1337") {
    ...
    console.log("Verifying EditionsFacet...");
    const artifact = await deployments.getArtifact("EditionsFacet");
