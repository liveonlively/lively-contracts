...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (OwnableFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("OwnableFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live && chainId !== "1337" && chainId !== "5777") {
    ...
    console.log("Verifying OwnableFacet...");
    const artifact = await deployments.getArtifact("OwnableFacet");
