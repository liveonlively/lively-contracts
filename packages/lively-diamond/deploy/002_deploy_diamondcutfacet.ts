...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (DiamondCutFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("DiamondCutFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live && chainId !== "1337") {
    ...
    const artifact = await deployments.getArtifact("DiamondCutFacet");
