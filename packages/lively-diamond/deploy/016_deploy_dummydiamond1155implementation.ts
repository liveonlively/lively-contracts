...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (DummyDiamond1155Implementation): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("DummyDiamond1155Implementation", deployOptions);
    } catch (e) {
  ...
  console.log("DummyDiamond1155Implementation: ", {
  ...
  if (network.live && chainId !== "1337") {
    ...
    console.log("Verifying DummyDiamond1155Implementation...");
    const artifact = await deployments.getArtifact("DummyDiamond1155Implementation");
