...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (QueryableFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("QueryableFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live && chainId !== "1337" && chainId !== "5777") {
    ...
    console.log("Verifying QueryableFacet...");
    const artifact = await deployments.getArtifact("QueryableFacet");
