...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (RoyaltyFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("RoyaltyFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live) {
    ...
    console.log("Verifying RoyaltyFacet...");
    const artifact = await deployments.getArtifact("RoyaltyFacet");
