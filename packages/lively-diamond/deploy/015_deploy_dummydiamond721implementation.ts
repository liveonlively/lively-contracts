...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (DummyDiamond721Implementation): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("DummyDiamond721Implementation", deployOptions);
    } catch (e) {
  ...
  console.log("DummyDiamond721Implementation: ", {
  ...
  if (network.live && chainId !== "1337") {
    ...
    console.log("Verifying DummyDiamond721Implementation...");
    const artifact = await deployments.getArtifact("DummyDiamond721Implementation");
