...
import { Lively1155DiamondABI } from "../../types";
...
describe(`DiamondBase 1155 Test`, function () {
  async function deployTokenFixture() {
    ...
    const diamondFacet = (await ethers.getContractAt("Lively1155DiamondABI", diamondAddress)) as Lively1155DiamondABI;
    ...
    const diamondUSDFacet = (await ethers.getContractAt(
      "Lively1155DiamondABI",
      diamondUSDAddress,
    )) as Lively1155DiamondABI;
  ...
  describe("Pack Functionality", function () {
    it("Should allow the owner to create a pack", async function () {
      ...
      const priceForAllTokens = defaultArgs._tokenData.reduce((acc, token) => {
        return acc.add(token.price as BigNumberish);
      }, ethers.BigNumber.from(0));

      const newPackPrice = priceForAllTokens.div(2); // Half off!

      // Owner should be able to create pack
      await expect(
        diamondFacet["packCreate(uint256[],uint256,uint256)"](
          [0, 1, 2],
          newPackPrice,
          0
        )
      ...
      await expect(
        diamondFacet
          .connect(signer1)
          ["packCreate(uint256[],uint256,uint256)"]([0, 1, 2], newPackPrice, 0)
      ).to.be.reverted;
      ...
      expect(
        await diamondFacet["balanceOf(address,uint256)"](signer1.address, 0)
      ).to.equal(1);
      expect(
        await diamondFacet["balanceOf(address,uint256)"](signer1.address, 1)
      ).to.equal(1);
      expect(
        await diamondFacet["balanceOf(address,uint256)"](signer1.address, 2)
      ).to.equal(1);
  ...
  describe("PaymentSplitterUpdate", function () {
    it("Should allow the owner to update the payment splitter", async function () {
      const { diamondFacet, owner, livelyDev, signer1, signer2 } =
      ...
      const tx = {
        to: diamondFacet.address,
        value: ethers.utils.parseEther("2"),
      };
      await signer2.sendTransaction(tx);

      // Both owner and livelyDev should have 1 ether
      expect(await diamondFacet["releasable(address)"](owner.address)).to.equal(
        ethers.utils.parseEther("1")
      );
      expect(
        await diamondFacet["releasable(address)"](livelyDev.address)
      ).to.equal(ethers.utils.parseEther("1"));
      expect(await diamondFacet["released(address)"](signer1.address)).to.equal(
        0
      );
      ...
      expect(
        await diamondFacet["releasable(address)"](signer1.address)
      ).to.equal(ethers.utils.parseEther("1"));
      expect(
        await diamondFacet["releasable(address)"](livelyDev.address)
      ).to.equal(ethers.utils.parseEther("1"));
      ...
      expect(
        await diamondFacet["releasable(address)"](livelyDev.address)
      ).to.equal(ethers.utils.parseEther("1.5"));
    });

    it("Should revert if the caller isn't a payee", async function () {
      const { diamondFacet, signer1, signer2 } = await loadFixture(
        deployTokenFixture
      );
      ...
      const contractInterface = new ethers.utils.Interface(errorInterface);

      await expect(
        diamondFacet
          .connect(signer1)
          .updatePaymentSplitterAddress(signer2.address)
  ...
  describe("Deployment", function () {
    ...
    it("Diamond should mint correctly", async function () {
      ...
      expect(
        await diamondFacet["balanceOf(address,uint256)"](owner.address, 0)
      ).to.equal(1);
    });

    it("Diamond should fail to mint if price is incorrect (and not owner)", async function () {
      const { owner, signer1, diamondFacet } = await loadFixture(
        deployTokenFixture
      );
      await expect(
        diamondFacet
          .connect(signer1)
          ["mint(address,uint256,uint256)"](owner.address, 0, 1, { value: 49 })
      ).to.be.reverted;
    });
    ...
    it("Diamond should fail to mint if the user pays for 1 while trying to buy 2", async function () {
      const { owner, signer3, diamondFacet } = await loadFixture(
        deployTokenFixture
      );
      ...
      await expect(
        diamondFacet
          .connect(signer3)
          ["mint(address,uint256,uint256)"](owner.address, 0, 2, {
    ...
    it("Diamond should succeed if minting two and paying the price for two", async function () {
      ...
      await expect(
        diamondFacet
          .connect(signer1)
          ["mint(address,uint256,uint256)"](owner.address, 0, 2, {
            value: (defaultArgs._tokenData[0].price as BigNumber).mul(2),
          })
      ).to.not.be.reverted;
    });
    ...
    it("Diamond should not allow the signer1 to mint for free", async function () {
      const { owner, diamondFacet, signer1 } = await loadFixture(
        deployTokenFixture
      );
      ...
      await expect(
        diamondFacet
          .connect(signer1)
          ["mint(address,uint256,uint256)"](owner.address, 0, 5, {
            value: 0,
          })
    ...
    it("Diamond should get the token metadata", async function () {
      const { diamondFacet } = await loadFixture(deployTokenFixture);
      expect(await diamondFacet["uri(uint256)"](0)).to.equal(
        "https://golive.ly/web3/meta/something/0"
      );
      expect(await diamondFacet["uri(uint256)"](1)).to.equal(
        "https://golive.ly/web3/meta/something/1"
      );
    });
    ...
    it("Diamond should allow the owner to airdrop a token by passing in an array of addresses", async function () {
      ...
      for (const wallet of randomWalletsAddresses) {
        expect(
          await diamondFacet["balanceOf(address,uint256)"](wallet, 2)
        ).to.equal(0);
      }

      await diamondFacet["mint(address[],uint256,uint256)"](
        randomWalletsAddresses,
        2,
        1,
        { gasLimit: 30000000 }
      );
    ...
    it("Diamond should allow the owner to airdrop to multiple multiple for free", async function () {
      const { signer1, signer2, signer3, diamondFacet } = await loadFixture(
        deployTokenFixture
      );
      ...
      expect(
        await diamondFacet["balanceOf(address,uint256)"](signer2.address, 0)
      ).to.equal(0);
      expect(
        await diamondFacet["balanceOf(address,uint256)"](signer3.address, 0)
      ).to.equal(0);
    ...
    it("Diamond should not allow a random user to airdrop to multiple multiple for free", async function () {
      ...
      await expect(
        diamondFacet
          .connect(signer1)
          ["mint(address[],uint256,uint256)"](recipients, 0, 1)
      ).to.be.reverted;
    ...
    it("Diamond should get the token metadata", async function () {
      const { owner, diamondFacet } = await loadFixture(deployTokenFixture);
      const tokenId = 0;
      const tx = await diamondFacet["mint(address,uint256,uint256)"](
        owner.address,
        tokenId,
        1,
        { value: 50 }
  ...
  describe("Create token function", function () {
    ...
    it("Should allow the owner to create multiple new tokens in a batch", async function () {
      ...
      await diamondFacet[
        "batchCreate(uint256,(uint256,uint256,address,string,bool,uint256,bool))"
      ](4, {
        maxSupply: 50,
        price: 100,
        tokenUri: "",
    ...
    it("Should allow owner to batch create tokens with different maxSupplies, price, tokenUris, and allowListEnabled", async function () {
      ...
      await diamondFacet[
        "batchCreate((uint256,uint256,address,string,bool,uint256,bool)[])"
      ](newTokenData);

      for (let i = startingEdition; i < newTokenData.length; i++) {
        expect(await diamondFacet.exists(i)).to.equal(true);
        expect(await diamondFacet.maxSupply(i)).to.equal(
          newTokenData[i].maxSupply
        );
        ...
        if (newTokenData[i].tokenUri) {
          expect(await diamondFacet["uri(uint256)"](i)).to.equal(
            newTokenData[i].tokenUri
          );
        } else {
          expect(await diamondFacet["uri(uint256)"](i)).to.equal(
            `${defaultArgs._baseURI}${i}`
          );
        }
      }
    });
  ...
  describe("Start time", function () {
    it("Should not allow the user to mint if it's before the correct time", async function () {
      ...
      await expect(
        diamondFacet
          .connect(signer1)
          ["mint(address,uint256,uint256)"](signer1.address, 2, 1, {
            value: 50,
          })
      ).to.be.reverted;
    });
    ...
    it("Should allow the owner to set a new time", async function () {
      ...
      await expect(diamondFacet.setStartTime(2, newStartTime)).to.not.be
        .reverted;
    ...
    it("Should not allow signer1 to change the startTime", async function () {
      ...
      const newStartTime = Math.floor(Date.now() / 1000) + 3600;
      await expect(diamondFacet.connect(signer1).setStartTime(2, newStartTime))
        .to.be.reverted;
    });
  ...
  describe("Allow list", function () {
    it("Should allow the allow list to be enabled and addresses added", async function () {
      ...
      await expect(diamondFacet.connect(signer1).enableAllowList(0)).to.be
        .reverted;
      ...
      await expect(
        diamondFacet
          .connect(signer1)
          ["addToAllowList(uint256,address,uint256)"](0, signer1.address, 2)
      ).to.be.reverted;

      const tx = diamondFacet["addToAllowList(uint256,address,uint256)"](
        0,
        signer1.address,
        2
      );
      ...
      // allowList = await diamondFacet.allowList();
  ...
  describe("EIP-165", function () {
    it("Should return the correct EIP-165 Standard Interface Detection", async function () {
      ...
      // 0x01ffc9a7 = ERC-165
      expect(
        await diamondFacet["supportsInterface(bytes4)"]("0x01ffc9a7")
      ).to.equal(true);
      // 0xd9b67a26 = ERC-1155
      expect(
        await diamondFacet["supportsInterface(bytes4)"]("0xd9b67a26")
      ).to.equal(true);
      // 0x1f931c1c = IDamondCut
      expect(
        await diamondFacet["supportsInterface(bytes4)"]("0x1f931c1c")
      ).to.equal(true);
      // 0x48e2b093 = IDiamondLoupe
      expect(
        await diamondFacet["supportsInterface(bytes4)"]("0x48e2b093")
      ).to.equal(true);
      // 0x7f5828d0 = IERC173
      expect(
        await diamondFacet["supportsInterface(bytes4)"]("0x7f5828d0")
      ).to.equal(true);
      // 0x0e89341c = IERC1155Metadata
      expect(
        await diamondFacet["supportsInterface(bytes4)"]("0x0e89341c")
      ).to.equal(true);
      // 0x2a55205a = IERC2981
      expect(
        await diamondFacet["supportsInterface(bytes4)"]("0x2a55205a")
      ).to.equal(true);
    });
  });
  ...
  describe("USD Tests", function () {
    it("Should return the price is USD", async function () {
      const { diamondUSDFacet, testUSDPrice, priceMock } = await loadFixture(
        deployTokenFixture
      );
