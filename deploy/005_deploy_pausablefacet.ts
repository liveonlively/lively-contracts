...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (PausableFacet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("PausableFacet", deployOptions);
    } catch (e) {
  ...
  if (network.live) {
    ...
    console.log("Verifying PausableFacet...");
    const artifact = await deployments.getArtifact("PausableFacet");
