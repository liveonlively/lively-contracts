...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  while (!newlyDeployed) {
    console.log("Deploy attempt (ERC1155Facet): ", deployAttempt++);
    try {
      newlyDeployed = await deploy("ERC1155Facet", deployOptions);
    } catch (e) {
  ...
  if (network.live) {
    ...
    console.log("Verifying ERC1155Facet...");
    const artifact = await deployments.getArtifact("ERC1155Facet");
